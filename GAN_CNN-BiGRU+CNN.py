...
def make_generator_model(input_dim, output_dim, feature_size) -> tf.keras.models.Model:
    ...
    model = Sequential()
    model.add(Conv1D(1024, input_shape=(input_dim, feature_size), kernel_size=3, strides=2, padding='same', activation=LeakyReLU(alpha=0.01)))
    model.add(Bidirectional(GRU(units=512, return_sequences = True, recurrent_dropout=0.2))) # 256, return_sequences = True
    model.add(GRU(units=256, recurrent_dropout=0.2)) #, recurrent_dropout=0.1
    model.add(GRU(units=128, recurrent_dropout=0.1))
...
class GAN:
    ...
    def train(self, real_x, real_y, yc, opt):
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % 15 == 0:
                tf.keras.models.save_model(generator, 'genCNN-biGRU_model_3_1_%d.h5' % epoch)
...
if __name__ == '__main__':
    ...
    ## For Bayesian
    opt = {"lr": 0.00016, "epoch": 330, 'bs': 128}
