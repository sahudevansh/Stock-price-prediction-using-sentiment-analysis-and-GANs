...
def make_discriminator_model():
    d = Sequential()
    d.add(Dense(32, input_dim=4, activation=LeakyReLU(alpha=0.01)))
    d.add(Dropout(0.2))
    d.add(Dense(64, activation=LeakyReLU(alpha=0.01)))
    d.add(Dropout(0.2))
    d.add(Dense(128, activation=LeakyReLU(alpha=0.01)))
    # d.add(Dropout(0.2))
    d.add(Flatten())
    d.add(Dense(220, use_bias=False))
    d.add(LeakyReLU())
    d.add(Dense(220, use_bias=False, activation='relu'))
...
class GAN:
    ...
    def train(self, real_x, real_y, yc, opt):
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % 15 == 0:
                tf.keras.models.save_model(generator, 'gen_GRUMLP_model_3_1_%d.h5' % epoch)
...
if __name__ == '__main__':
    ...
    ## For Bayesian
    opt = {"lr": 0.00016, "epoch": 400, 'bs': 128}
